...
/*
    ...
    **/
    private boolean isPalindromeThird(Node<Integer> head) {
        ...
        if (head == null) return true;              // 修正   再修正 true，不是false，没有值也算回文
        ...
        /**
         * 修正，我原先用的是 head.getNext() != null，
         * 就变成了比对最后一位的值。 再修正，一开始用的index1 != null 会报错。
         * index1 遍历比index2多一位，这里面index1 和index2存在交点。
         *
         * 再修正！ 我在网页上自己写的时候判断条件写的是index2.next,如果像我这么写的话，那么无法比较两链表的最后一位。
         */
